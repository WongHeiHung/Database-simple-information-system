...
// Please give me A+ in this course, hehe!
...
void showTeach() {
	...
	SQLLEN staff_id_n = SQL_NTS;

	SQLRETURN retcode = SQLPrepareA(hstmt, (SQLCHAR*)
		...
		"AND pt.staff_id = o.staff_id "
		"AND pt.staff_id = ?;",
		SQL_NTS);

	retcode = SQLBindParameter(hstmt, 1, SQL_PARAM_INPUT, SQL_C_SLONG, SQL_INTEGER, 0, 0, &staff_id, 0, &staff_id_n); 
	retcode = SQLExecute(hstmt);

	SQLCHAR*** results = printIntoRow(hstmt, 20);
...
void showPre() {
	...
	SQLRETURN retcode = SQLPrepareA(hstmt, (SQLCHAR*)
		...
		"ORDER BY main_course_ID;", SQL_NTS);
...
void addPhone() {
	...
	SQLRETURN  retcode = SQLPrepareA(hstmt, (SQLCHAR*) "SELECT COUNT(*) FROM prof_phone WHERE phone_number = ? AND staff_id = ?;", SQL_NTS);

	retcode = SQLBindParameter(hstmt, 1, SQL_PARAM_INPUT, SQL_C_SLONG, SQL_INTEGER, 0, 0, &newPhone, 0, NULL);
	retcode = SQLBindParameter(hstmt, 2, SQL_PARAM_INPUT, SQL_C_SLONG, SQL_INTEGER, 0, 0, &staff_id, 0, &staff_id_n);
	...
	SQLINTEGER count;  
	SQLLEN countLength;  
	SQLBindCol(hstmt, 1, SQL_C_SLONG, &count, 0, &countLength);
	SQLFetch(hstmt);
	...
	if (count > 0) {
		...
		retcode = SQLPrepareA(hstmt, (SQLCHAR*)"insert into prof_phone values(?, ?);", SQL_NTS);

		retcode = SQLBindParameter(hstmt, 1, SQL_PARAM_INPUT, SQL_C_SLONG, SQL_INTEGER, 0, 0, &staff_id, 0, &staff_id_n);
		retcode = SQLBindParameter(hstmt, 2, SQL_PARAM_INPUT, SQL_C_SLONG, SQL_INTEGER, 0, 0, &newPhone, 0, NULL);
